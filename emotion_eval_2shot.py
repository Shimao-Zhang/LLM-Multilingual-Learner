...
for step, inputs in enumerate(sample_iterator):
    ...
    if (target_lang not in ['th'] and "mistral" in model_size) or (("mistral" not in model_size) and (target_lang not in ['th'])):
        ...
        if all_positive_out_probs_sum > all_negative_out_probs_sum:
            output = [input[0] + positive_answers[target_lang]]
            # break
        elif all_positive_out_probs_sum < all_negative_out_probs_sum:
            output = [input[0] + negative_answers[target_lang]]
            # break
        else:
        ...
        logits = model.forward(torch.cat((input_tokens['input_ids'], tokenizer("เชิง", return_tensors='pt')['input_ids'].to("cuda")), dim=-1)).logits
        probs = Func.softmax(logits, dim=-1)
        all_positive_out_ids = process_tokens("บวก", tokenizer)
        all_negative_out_ids = process_tokens("ลบ", tokenizer)
        all_positive_out_probs_sum = sum([probs[0][-1][i].item() for i in all_positive_out_ids])
        all_negative_out_probs_sum = sum([probs[0][-1][i].item() for i in all_negative_out_ids])
        if all_positive_out_probs_sum > all_negative_out_probs_sum:
            output = [input[0] + positive_answers[target_lang]]
    ...
    # print(output)
    for i in range(len(output)):
        if (output[i].startswith(inputs['answer'][i])):
            correct_num += 1
        elif not (output[i].startswith(positive_answers[target_lang]) or output[i].startswith(negative_answers[target_lang])):
